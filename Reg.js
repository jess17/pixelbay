...
function checkEmpty() {
  ...
  for (i = 0; i < 4; i++) {
    if (userLength == 0 && !(arr.includes(1))) {
      getElByClass('error', 0).innerHTML = "Please field out this field";
      userErr();
      arr[0] = 1;
      // alert(arr);
    }
    else if (emailLength == 0 && !(arr.includes(2))) {
      getElByClass('error', 1).innerHTML = "Please field out this field";
      emailErr();
      arr[1] = 2;
    }
    else if (passLength == 0&& !(arr.includes(3))) {
      getElByClass('error', 2).innerHTML = "Please field out this field";
      passErr();
      arr[2]=3;
    }
    else if (confPassLength == 0 && !(arr.includes(4))) {
      getElByClass('error', 3).innerHTML = "Please field out this field";
      confPassErr();
      arr[3]=4;
    }
