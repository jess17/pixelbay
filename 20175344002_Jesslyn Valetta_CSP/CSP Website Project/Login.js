...
let writeCookie = function (){
  let cookieVal = encodeURI(getElByClass('username',0).value);
...
function cookie(){
  if(document.getElementById("rememberMe").checked == true){
    // let btn = document.getElByClass("checkEmpty", 0);
    // btn.addEventListener('click', writeCookie);
    writeCookie();
  }
}
getElByClass("checkEmpty", 0).addEventListener('click', cookie);
